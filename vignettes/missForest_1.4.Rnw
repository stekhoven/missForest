...
%\VignetteIndexEntry{missForest_1.4.Rnw}
%\VignetteDepends{randomForest,doParallel,foreach,itertools}
...
\date{Friday, 13$^{\textrm{th}}$ of May, 2011\\\small{Update: Version 1.4, 30.12.13}}
\begin{document}
...
The following code is loading the \texttt{doParallel} package and registers a (default) parallel backend. The function \texttt{getDoParWorkers()} returns the number of available cores.

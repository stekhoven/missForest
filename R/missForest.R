...
{ ## ----------------------------------------------------------------------
  ...
  .sample_fraction <- function(sampsize_i, y, replace_flag, n_obs){
    if (is.null(sampsize_i)) return(if (replace_flag) 1 else 0.632)
    ...
    if (replace_flag) 1 else 0.632
  }
  
  ## stop in case of wrong inputs passed
  n <- nrow(xmis)
  p <- ncol(xmis)
  if (!is.null(classwt)) stopifnot(length(classwt) == p, typeof(classwt) == 'list')
  if (!is.null(cutoff))  stopifnot(length(cutoff) == p,  typeof(cutoff)  == 'list')
  if (!is.null(strata))  stopifnot(length(strata) == p,  typeof(strata)  == 'list')
  if (!is.null(nodesize)) stopifnot(length(nodesize) == 2)
  ...
  if (parallelize %in% c('variables', 'forests')) {
    if (foreach::getDoParWorkers() == 1) {
      ...
      if (parallelize == 'variables') cat("  parallelizing over variables\n")
      if (parallelize == 'forests')   cat("  parallelizing forest building\n")
    }
  ...
  while (stopCriterion(varType, convNew, convOld, iter, maxiter)){
    ...
    if (parallelize == 'variables'){
      for (idx in idxList) {
        results <- foreach::foreach(varInd = idx,
                                    .packages = if (backend == 'ranger') 'ranger' else 'randomForest') %cols% {
                                      obsi <- !NAloc[, varInd]; misi <- NAloc[, varInd]
                                      ...
                                      if (backend == 'ranger'){
                                        ## ranger path
                                        sf <- .sample_fraction(if (!is.null(sampsize)) sampsize[[varInd]] else NULL,
                                                               y = if (typeY == 'factor') factor(obsY) else obsY,
                                                               replace_flag = replace, n_obs = nrow(obsX))
                                        if (typeY == 'numeric'){
                                          RF <- ranger::ranger(x=obsX, y=obsY, num.trees=ntree, mtry=mtry,
                                                               replace=replace, sample.fraction=sf,
                                                               min.bucket = if (!is.null(nodesize)) nodesize[1] else 5,
                                                               write.forest=TRUE, oob.error=TRUE, num.threads=1,
                                                               verbose=FALSE)
                                          oerr <- RF$prediction.error
                                          misY <- ranger::predict(RF, data = misX)$predictions
                                        } else {
                                          obsY <- factor(obsY); summarY <- summary(obsY)
                                        ...
                                        if (typeY == 'numeric'){
                                          RF <- randomForest::randomForest(x=obsX, y=obsY, ntree=ntree, mtry=mtry,
                                                                           replace=replace,
                                                                           sampsize = if (!is.null(sampsize)) sampsize[[varInd]] else if (replace) nrow(obsX) else ceiling(0.632*nrow(obsX)),
                                          ...
                                          if (length(summarY) == 1){ oerr <- 0; misY <- factor(rep(names(summarY), length(which(misi)))) }
                                            ...
                                            RF <- randomForest::randomForest(x=obsX, y=obsY, ntree=ntree, mtry=mtry,
                                                                             replace=replace,
                                                                             classwt = if (!is.null(classwt)) classwt[[varInd]] else rep(1, nlevels(obsY)),
                                                                             cutoff = if (!is.null(cutoff)) cutoff[[varInd]] else rep(1/nlevels(obsY), nlevels(obsY)),
    ...
    } else { ## sequential across variables, optional forest-level parallel
      for (s in 1:p) {
        varInd <- sort.j[s]
        if (noNAvar[[varInd]] == 0) next
        obsi <- !NAloc[, varInd]; misi <- NAloc[, varInd]
        obsY <- ximp[obsi, varInd]; obsX <- ximp[obsi, seq_len(p)[-varInd]]
        misX <- ximp[misi, seq_len(p)[-varInd]]; typeY <- varType[varInd]
        ...
        if (backend == 'ranger'){
          ...
          if (typeY == 'numeric'){
            ...
            obsY <- factor(obsY); summarY <- summary(obsY)
            if (length(summarY) == 1){ misY <- factor(rep(names(summarY), sum(misi))); OOBerror[varInd] <- 0 }
            else {
              use_prob <- !is.null(cutoff) && !is.null(cutoff[[varInd]])
              RF <- ranger::ranger(x=obsX, y=obsY, num.trees=ntree, mtry=mtry, replace=replace,
                                   sample.fraction=sf,
                                   class.weights = if (!is.null(classwt)) classwt[[varInd]] else NULL,
                                   min.bucket = if (!is.null(nodesize)) nodesize[2] else 1,
                                   write.forest=TRUE, oob.error=TRUE, probability=use_prob,
                                   respect.unordered.factors = "order",
              ...
              if (use_prob){
                probs <- ranger::predict(RF, data = misX)$predictions
                lev <- colnames(probs); co <- cutoff[[varInd]]; if (is.null(names(co))) names(co) <- lev; co <- co[lev]
                idxc <- max.col(sweep(probs, 2, co, "/"), ties.method = "first")
                misY <- factor(lev[idxc], levels = lev)
              } else {
                misY <- ranger::predict(RF, data = misX)$predictions
          ...
          if (typeY == 'numeric'){
            if (parallelize == 'forests'){
              RF <- foreach::foreach(xntree = iterators::idiv(ntree, chunks = foreach::getDoParWorkers()),
                                     ...
                                     .packages='randomForest') %dorng% {
                                       randomForest::randomForest(x=obsX, y=obsY, ntree=xntree, mtry=mtry, replace=replace,
                                                                  sampsize = if (!is.null(sampsize)) sampsize[[varInd]] else if (replace) nrow(obsX) else ceiling(0.632*nrow(obsX)),
                                                                  nodesize = if (!is.null(nodesize)) nodesize[1] else 1,
                                                                  maxnodes = if (!is.null(maxnodes)) maxnodes else NULL)
                                     }
              ...
              RF <- randomForest::randomForest(x=obsX, y=obsY, ntree=ntree, mtry=mtry, replace=replace,
                                               sampsize = if (!is.null(sampsize)) sampsize[[varInd]] else if (replace) nrow(obsX) else ceiling(0.632*nrow(obsX)),
                                               nodesize = if (!is.null(nodesize)) nodesize[1] else 1,
                                               maxnodes = if (!is.null(maxnodes)) maxnodes else NULL)
            ...
            if (length(summarY) == 1){ misY <- factor(rep(names(summarY), sum(misi))); OOBerror[varInd] <- 0 }
              ...
              if (parallelize == 'forests'){
                ...
                ne <- as.integer(predict(RF)) != as.integer(RF$y); ne <- ne[!is.na(ne)]
    ...
    if (any(!is.na(xtrue))) { err <- suppressWarnings(mixError(ximp, xmis, xtrue)) }
    if (verbose){
      delta.start <- proc.time() - t.start
      if (any(!is.na(xtrue))) cat("    error(s):", err, "\n")
